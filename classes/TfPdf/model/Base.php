...
class TfPdf_Base {
    ...
    var $fontcache;          // path containing font cache ( generated PHP and DAT files )
    ...
    public function __construct($orientation='P', $unit='mm', $size='A4') {
        ...
        // Font path
        if (!is_object( App_Application::getInstance()->getConfig()->tfpdf ) )
                throw new TfPdf_Exception( 'TFPDF is not configured' );

        $this->fontpath = App_Application::getInstance()->getConfig()->tfpdf->fontpath;
        if ( $this->fontpath == '' || !is_dir( $this->fontpath ))
                throw new TfPdf_Exception( 'TFPDF:invalid font path specified' );
        if (substr($this->fontpath, -1) != '/' && substr($this->fontpath, -1) != '\\')
                $this->fontpath .= '/';
        
        $this->fontcache = App_Application::getInstance()->getConfig()->tfpdf->fontcache;
        if ( $this->fontcache == '' || !is_dir( $this->fontcache ))
                throw new TfPdf_Exception( 'TFPDF:invalid font cache path specified' );
        if (substr($this->fontcache, -1) != '/' && substr($this->fontcache, -1) != '\\')
                $this->fontcache .= '/';
    ...
    function Error($msg) {
        // Fatal error
        throw new TfPdf_Exception('<b>FPDF error:</b> ' . $msg);
    }
    ...
    function AddFont($family, $style='', $file='', $uni=false) {
        ...
        if ($uni) {
            ...
            $unifilename = $this->_getfontcache() . 'unifont/' . strtolower(substr($file, 0, (strpos($file, '.'))));
            $name = '';
            $originalsize = 0;

            if ( !file_exists( $ttffilename ))
                throw new TfPdf_Exception( 'TTF File Is Missing: ' . $ttffilename );
            ...
            if (!isset($type) || !isset($name) || $originalsize != $ttfstat['size']) {
                ...
                $s.='$fontkey=\'' . $fontkey . "';\n";
                $s.="";
                if (is_writable(dirname($this->_getfontcache() . 'unifont/' . 'x'))) {
                ...
                if ( file_exists( $unifilename . '.cw.dat' ) )
